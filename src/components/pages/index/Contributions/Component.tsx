...
const months = [...monthNames.slice(currentMonth, 12), ...monthNames.slice(0, currentMonth + 1)];
...
const Contributions = async () => {
  ...
  return (
    <div id="scss-contributions-ssr-r">
      <div>{contributions.totalContributions} contributions in the last year</div>
      <div className="main">
                ...
                .map((_, weekDay) => (
                  <tr key={weekDay}>
                    {contributions.weeks.map((week: any, index: number) => {
                      const filtered = week.contributionDays.find(
                        (day: any) => day.weekday === weekDay
                      );
                      const level = getContributionLevel(filtered?.contributionCount);
...
function getContributionLabel(contribution: any) {
  ...
  if (contribution.contributionCount >= 2)
    resp[0] = `${contribution.contributionCount} contribuitions`;
