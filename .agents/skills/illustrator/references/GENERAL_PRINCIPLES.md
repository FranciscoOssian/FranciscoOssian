# General Principles for Software Illustration
...
- **Aspect Ratio**: You MUST choose from these specific ratios:
  ...
  - **3:4**: Portrait.
  - **9:16**: Vertical (Full mobile story style).
- **Scale & Resolution**:
  - Request high resolution (e.g., 2048px width) for Heros to avoid pixelation.
  - Consider the final file format (PNG for transparency, JPG for photographs/paintings, SVG-style for icons).
- **Responsiveness**:
  - Small sizes: Reduce visual complexity (fewer elements) to remain readable.
  - Large sizes: Use the extra space for textures or secondary brand details.
- **Placement Context**: Check if the image will have text overlaid (requires clear space in the composition).
